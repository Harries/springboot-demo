 ...
 * </copyright>
 */
package demo.hankcs.book.ch11;

import demo.hankcs.hanlp.classification.classifiers.IClassifier;
import demo.hankcs.hanlp.classification.classifiers.NaiveBayesClassifier;
import demo.hankcs.hanlp.classification.corpus.FileDataSet;
import demo.hankcs.hanlp.classification.corpus.IDataSet;
import demo.hankcs.hanlp.classification.corpus.MemoryDataSet;
import demo.hankcs.hanlp.classification.statistics.evaluations.Evaluator;
import demo.hankcs.hanlp.classification.statistics.evaluations.FMeasure;
import demo.hankcs.hanlp.classification.tokenizers.BigramTokenizer;
import demo.hankcs.hanlp.classification.tokenizers.HanLPTokenizer;
import demo.hankcs.hanlp.classification.tokenizers.ITokenizer;
import demo.hankcs.demo.DemoTextClassification;
...
public class DemoTextClassificationFMeasure
{
    ...
    public static void evaluate(IClassifier classifier, ITokenizer tokenizer) throws IOException
    {
        IDataSet trainingCorpus = new FileDataSet().                          // FileDataSet省内存，可加载大规模数据集
            ...
            load(DemoTextClassification.CORPUS_FOLDER, "UTF-8", 0.9);               // 前90%作为训练集
        classifier.train(trainingCorpus);
        IDataSet testingCorpus = new MemoryDataSet(classifier.getModel()).
            load(DemoTextClassification.CORPUS_FOLDER, "UTF-8", -0.1);        // 后10%作为测试集
        // 计算准确率
